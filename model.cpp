...
struct Vertex {
    unsigned int v, vt, vn; // pozycja wierzcho³ka, wspó³rzêdne tekstury, normalna
};
...
bool loadOBJ(const char* filename, std::vector<float>& vertices, std::vector<float>& normals, std::vector<float>& texCoords) {
    ...
    while (std::getline(file, line)) {
        std::istringstream iss(line); // strumieñ danych
        std::string token; // okreœla typ danych zawartych w linii
        iss >> token; // wczytanie pierwszego tokenu z linii
        ...
        if (token == "v") { // vertex
            ...
            while (iss >> vertexString) { // kolejne wierzcho³ki œciany
                Vertex vertexIndex;
            ...
            // zamiana wielok¹tów na trójk¹ty
    ...
    // zapisanie wszystkich danych w przeznaczonych wektorach
